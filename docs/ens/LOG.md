...
- Merged `origin/main` (PR #12, Dean's World docs) into `qa/final` with a merge commit, no conflicts. PR #12 was written before the switch, so it still said `setVerifier` was pending. Minimal factual fix in `deployments.json` (`sepoliaWorldIdV4Wallet.setVerifier` = the tx, plus `setVerifierBlock` / `setVerifierFrom`; the first gate's note says superseded), `docs/WORLD.md` and `docs/BEKTUR-WORLD-V4.md` (which gate is live, no other changes).
...
- Checks at 12:53 JST: root forge 163/163 (12 suites); app `tsc` clean, production build, vitest 95/95, `live-smoke` all wiring checks passed (`humanGate.verifier()` = `0x5Cb8…aABa`, `isVerified(alice)` = `true`, first gate listed as superseded; a scratch copy with a mismatched `setVerifierBlock` fails as intended), `ens-smoke` OK.
