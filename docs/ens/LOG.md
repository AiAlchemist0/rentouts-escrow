...
**Sat 12:47–12:51: `judge.rentouts.eth` and `EnsAgentRelay` live; ENS now gates the AI judge on-chain (branch `feat/ens-judge-name`).** Times are block timestamps (`cast block <n> --field timestamp`).
- 12:47:24, block 11783660: the deployer (an issuer) sent `RentoutsSubnames.register("judge", 0x4a44…d0dA)` ([`0x5454ab9b…4e73`](https://sepolia.etherscan.io/tx/0x5454ab9bf9e6c62ba94726628daf159fbfd087aa24cdbd39f21718b72e8d4e73), status 1, 329,847 gas). `judge.rentouts.eth` resolves to the judge key through the Universal Resolver.
- 12:47:48 and 12:48:00, blocks 11783662–3: the judge key set its own profile, `description` ([`0x16a40427…4960`](https://sepolia.etherscan.io/tx/0x16a4042791ee61024c1cd6146de5de3311b234702a99c9fbbac81bd61cbf4960)) and `url` ([`0x24546fad…5871`](https://sepolia.etherscan.io/tx/0x24546fadd4230623002ee060d65a51d2c9c9dc5e5cde9fe03b785cb0ac3e5871)). Receipts and `ens/deployments/sepolia.json` (`judgeName`, `judgeHolder`) committed in `acccfbd`.
- 12:50:36, block 11783676: the deployer deployed `EnsAgentRelay` `0xe56E49cAA4780B71F667bF08a9ADb2C659d9C3eE` ([`0xabc6a72d…a570`](https://sepolia.etherscan.io/tx/0xabc6a72d69a5063d2ce3593c390daf5de2c085e6d0a20f87b2b3e4866ccba570), 860,147 gas). Sourcify `exact_match` (creation and runtime), Blockscout verified.
- 12:51:00, block 11783678: the human arbiter `0x798b…e486` called `AIArbiter.setAgent(relay)` ([`0x0fc2c12c…bb44ad`](https://sepolia.etherscan.io/tx/0x0fc2c12c8686c3b24ee9435a560cc9e795ae675eb057095066969b2ce3bb44ad), `AgentUpdated(0x4a44…d0dA → 0xe56E…C3eE)`). From here the judge key can propose only through the relay and only while it holds `judge.rentouts.eth`. The judge runs with `JUDGE_RELAY=0xe56E49cAA4780B71F667bF08a9ADb2C659d9C3eE ./run.sh --lease <id> --propose`. Rollback = `setAgent(0x4a44…d0dA)` by the human (`../sign-judge-name.sh rollback`). Recorded in `deployments.json` (`sepoliaEnsAgentRelay`) in `d92f140`.
- The live name must never be revoked: labels are single-use, so `judge` could not be issued again. The revoke case is a fork test only.
...
**Sat 12:52: combined gate re-pointed at the live World gate #2 (branch `feat/ens-world-gate`, still ready to deploy, NOT broadcast).**
- Live since 12:42: `HumanGate.verifier()` = `WorldIdV4Gate` #2 `0x5Cb885E6292003492932f3fa647A9d6Bf8A4aABa` (action `fund-lease-wallet`), owner tx `0xcd93549e9a3a703be498b96bd6ad47afd46c1d332a637460f4b94e127eb86671`, block 11783640. alice is registered on it (tx `0xdbbfc6dd…8908`). Gate #1 `0x27052bD6…B209` is superseded.
- `script/DeployEnsWorldGate.s.sol`: `WORLD_GATE` now defaults to gate #2; the printed owner commands are copy-paste ready (`--account rentouts-deployer`, public RPC) and warn if `WORLD_GATE` is not the current verifier.
...
**Sat 12:56: alice topped up to 3.0 test USDC.** 1.0 USDC from the deployer, [`0xb315f812…a170`](https://sepolia.etherscan.io/tx/0xb315f812ad413cc9b4bd9115b040719bb903303db7e706bcc4c644168515a170) (block 11783705, 12:56:24, status 1). Alice now holds 3.0 USDC and 0.01 ETH, enough for the four demo leases with 1.0 USDC of slack. The deployer holds 0 USDC.
...
**Sat 13:20–13:45: `feat/ens-judge-name` merged with `main`; the docs and checks match the live chain.**
- Merged `origin/main` `8095d58` (PRs #9–#18) into the branch with a merge commit. Conflicts: `deployments.json` takes `main`'s World entries unchanged (Dean's) and keeps `sepoliaEnsAgentRelay`; this log keeps every entry from both sides in time order; `judge/README.md` count.
- `deployments.json`: `sepoliaAIArbiter.agent` is now the relay `0xe56E…C3eE` (the on-chain agent), with `agentKind`, `agentSince` (the `setAgent` tx), `agentSinceBlock` 11783678, `agentSetBy`, `judgeKey` `0x4a44…d0dA`, `judgeName` and `initialAgent` (the key it was deployed with). `sepoliaEnsAgentRelay` gains the deploy tx and block, verification, `judge`, the register tx, `status` and `rollback`.
- The two fork tests that assumed the pre-broadcast state now test the live state: `test/EnsAgentRelay.fork.t.sol` (7) proposes a real lease through the live relay, checks the non-holder and direct-call refusals, the revoke (fork only), rollback, `agent()` = relay, and that the deploy script builds the live relay's exact runtime code; `ens/test/JudgeName.fork.t.sol` (10) checks the live name, relay, profile, soulbound, holder-only profile, re-runs that send nothing, rollback, and the revoke on the fork.
- `app/scripts/live-smoke.mjs` now fails unless `AIArbiter.agent()` is the recorded `sepoliaAIArbiter.agent` and `judge.rentouts.eth` resolves to the relay's judge (= the recorded `judgeKey`); a scratch copy with the old agent fails as intended.
