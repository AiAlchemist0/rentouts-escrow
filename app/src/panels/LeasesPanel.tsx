...
import { useContracts, useLeases, useNow, useRentoutsNames, useTx, useWallet, type LeaseRow } from '../hooks'
...
function LeaseCard({ lease, account }: { lease: LeaseRow; account: Address | undefined }) {
  const { escrow, arbiter, tokenDecimals, tokenSymbol } = useContracts()
  const nameOf = useRentoutsNames([lease.landlord, lease.tenant])
  const now = useNow()
  const tx = useTx()
  ...
  const role = isLandlord ? 'You’re the landlord' : isTenant ? 'You’re the tenant' : isArbiter ? 'You’re the arbiter' : null
  ...
  } else if (lease.state === LeaseState.DISPUTED) clock = 'Frozen until the arbiter resolves the dispute.'
  ...
  return (
    <li className={`lease lease-${STATE_LABELS[lease.state]?.toLowerCase().replace(/\s+/g, '-')}`}>
      ...
      {lease.state === LeaseState.DISPUTED && !isArbiter && arbiter ? (
        <p className="hint">
          Arbiter: <AddressLink address={arbiter} />. A single test account here; a Safe multisig in production.
        </p>
      ) : null}
...
export function LeasesPanel() {
  ...
  const isArbiter = same(address, arbiter)
  const mine = all.filter((l) => same(address, l.landlord) || same(address, l.tenant))
  const visible = showAll || !address ? all : isArbiter ? all.filter((l) => l.state === LeaseState.DISPUTED).concat(mine.filter((l) => l.state !== LeaseState.DISPUTED)) : mine
    ...
    <div className="stack">
      <div>
        ...
        <p className="lede">
          ...
          lease closes: remaining rent to the landlord, deposit back to the tenant. Either side can freeze it for the
          arbiter instead.
        </p>
      </div>
      ...
      {!escrow ? (
        ...
        <>
          ...
          {visible.length === 0 ? (
            ...
            <ul className="list">
              {visible.map((lease) => (
                <LeaseCard key={String(lease.id)} lease={lease} account={address} />
              ))}
            </ul>
          )}
        </>
