...
| World ID AND ENS combined gate (not deployed) | `src/EnsCredentialGate.sol`, `src/AllOfHumanGate.sol`, `test/EnsWorldGate*`, `test/helpers/MockGates.sol`, `script/DeployEnsWorldGate.s.sol` | Bektur with Claude Code |
| ENS identity | `ens/` (see [`ens/AI_USAGE.md`](./ens/AI_USAGE.md)) | Bektur with Claude Code |
| AI dispute judge service | `judge/`, including the Tokyo rules pack `judge/src/rules/tokyo.ts` | Bektur with Claude Code |
...
| World ID | `src/WorldIdV4Gate.sol`, `src/WorldHumanVerifier.sol`, their tests and deploy scripts, `docs/WORLD.md`, `docs/BEKTUR-WORLD-V4.md` | Dean (squash-merged PRs #9–#13). _Dean to confirm the tools used._ The extra security tests `test/WorldIdV4GateSecurity.t.sol` and the app's World ID wiring came from Bektur's QA pass with Claude Code (PR #14). |
...
- **Every change** after the three setup commits reached `main` through a pull request (#1 onward), merged by a team member.
...
- It classifies each claimed item against a versioned Tokyo restoration rules pack (ageing, normal use or tenant damage) and cites the rule ids.
