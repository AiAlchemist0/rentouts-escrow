...
ens/
  ...
  script/JudgeName.s.sol           judge.rentouts.eth for the AI judge: judgeStatus | registerJudge | judgeProfile
  ...
  test/JudgeName.fork.t.sol        the live judge.rentouts.eth: resolution, relay, profile, soulbound, re-runs; revoke on the fork
...
The AI dispute judge (`judge/`) signs with the key `0x4a444685F3E700D0d5B8Fe53d987f8029cced0dA`. ENS names it: `judge.rentouts.eth` is a RentOuts subname like a tenant's, soulbound and revocable, and it resolves through the Universal Resolver to that key. **Live since Sat 12:51 JST, the name is what lets the AI propose:** `AIArbiter.agent()` is the `EnsAgentRelay` [`0xe56E49cAA4780B71F667bF08a9ADb2C659d9C3eE`](https://eth-sepolia.blockscout.com/address/0xe56E49cAA4780B71F667bF08a9ADb2C659d9C3eE), which forwards proposals only from the name's current holder. Remove ENS and the AI can't propose. Its profile says what it does: `description` = "RentOuts AI dispute judge: proposes rulings on AIArbiter 0xC3D5…bdCb5; humans can appeal and override", `url` = the repo.

- **Live transactions** (Sepolia, block times in JST): `register("judge", 0x4a44…d0dA)` by the deployer (an issuer) in [`0x5454ab9b…4e73`](https://sepolia.etherscan.io/tx/0x5454ab9bf9e6c62ba94726628daf159fbfd087aa24cdbd39f21718b72e8d4e73) (block 11783660, 12:47:24); `description` in [`0x16a40427…4960`](https://sepolia.etherscan.io/tx/0x16a4042791ee61024c1cd6146de5de3311b234702a99c9fbbac81bd61cbf4960) (block 11783662, 12:47:48) and `url` in [`0x24546fad…5871`](https://sepolia.etherscan.io/tx/0x24546fadd4230623002ee060d65a51d2c9c9dc5e5cde9fe03b785cb0ac3e5871) (block 11783663, 12:48:00), both by the judge key; `EnsAgentRelay` deployed by the deployer in [`0xabc6a72d…a570`](https://sepolia.etherscan.io/tx/0xabc6a72d69a5063d2ce3593c390daf5de2c085e6d0a20f87b2b3e4866ccba570) (block 11783676, 12:50:36; Sourcify `exact_match`, Blockscout verified); `AIArbiter.setAgent(relay)` by the human arbiter `0x798b…e486` in [`0x0fc2c12c…bb44ad`](https://sepolia.etherscan.io/tx/0x0fc2c12c8686c3b24ee9435a560cc9e795ae675eb057095066969b2ce3bb44ad) (block 11783678, 12:51:00). The state file records `judgeName` / `judgeHolder`; the root `deployments.json` records the relay (`sepoliaEnsAgentRelay`) and `sepoliaAIArbiter.agent` = the relay with `judgeKey` = `0x4a44…d0dA`.
- **Issue it** (`script/JudgeName.s.sol`, same env and state file as `DeployEns`): `registerJudge()` is sent by the issuer and mints the name to `AIArbiter.agent()`, read on chain, never hard-coded (when the agent is already the relay, it reports the relay's judge, sends nothing and records it). `judgeProfile()` is sent by the **judge key**, because `RentoutsSubnames.setProfileText` is holder-only (the issuer can't write a holder's profile). So it takes two signers: `rentouts-deployer`, then `rentouts-judge`. The state file gets `judgeName` / `judgeHolder` only once the chain shows the name (a re-run records it; no transaction). `judgeStatus()` is a read-only report. The guided signer is `../sign-judge-name.sh` (`FORK=1` rehearses it on an anvil fork).
- **The judge checks it before every proposal** (`judge/src/ens.ts`): `judge.rentouts.eth` must resolve to the signing key, and that key must be `AIArbiter.agent()`, or, with `JUDGE_RELAY=0xe56E…C3eE` (needed since 12:51 JST), the relay's `judge()` while the relay is the agent. If it doesn't, the judge refuses to send. `JUDGE_ENS_NAME` changes the name, and `off` disables the check for local mock runs.
- **The app shows it** (`app/src/panels/AiJudgePanel.tsx`): "Proposed by judge.rentouts.eth ✓" when the name resolves to the proposing agent, or to the relay's judge when the agent is the relay; otherwise the raw address.
- **On-chain gate, live** (`../src/EnsAgentRelay.sol`): the human arbiter made `EnsAgentRelay` AIArbiter's agent (`setAgent`, no redeploy). The relay forwards `propose` only from the current holder of `judge.rentouts.eth`: `RentoutsSubnames.holderOf(labelId)` and the ENS registry owner must both be the caller. Revoking the name would stop the AI at once, and the judge key can't go around it (a direct `AIArbiter.propose` reverts `NotAgent`). The judge runs with `JUDGE_RELAY=0xe56E49cAA4780B71F667bF08a9ADb2C659d9C3eE`. Rollback is `setAgent(0x4a444685F3E700D0d5B8Fe53d987f8029cced0dA)` by the human. Labels are single-use, so a revoked `judge` label can't be reissued: a new judge key would need a new label and a new relay. **The live name is never revoked**; the revoke case runs on a fork only.
- **Tests:** `test/JudgeName.fork.t.sol` (10 tests) checks the live name on a fork of Sepolia: the Universal Resolver returns the judge key, `AIArbiter.agent()` is the relay and `relay.judge()` is the name's address, the texts and the state file match, transfers revert (soulbound), only the judge key writes its profile, the resolution check rejects any other address (the relay included), re-runs of `registerJudge` / `judgeProfile` send nothing and record the holder only on a broadcast, rollback, and (fork only) that a revoke empties the name and the relay's judge and retires the label. Before the broadcast, the same file rehearsed the fresh registration (see `docs/ens/LOG.md`, Sat 12:44). The relay has 13 unit tests and 7 fork tests against the live relay in the root project (`test/EnsAgentRelay*.t.sol`).
